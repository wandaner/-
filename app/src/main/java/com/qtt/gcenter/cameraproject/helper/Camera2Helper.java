package com.qtt.gcenter.cameraproject.helper;
...
public class Camera2Helper {
    ...
    private static final String TAG_CAMERA = "CameraBackground";

    private Context context;

    private int mCameraId = LENS_FACING_FRONT; // 要打开的摄像头ID
    ...
    private Matrix mFaceDetectMatrix;                                           //人脸检测坐标转换矩阵
    private ArrayList<RectF> mFacesRect;                                        //保存人脸坐标信息
    //    private FaceDetectListener mFaceDetectListener;                           //人脸检测回调

    public Camera2Helper(Context activity, boolean isFrontCamera) {
        this.context = activity;
        this.isFrontCamera = isFrontCamera;
        //        this.mCameraId = isFrontCamera ? LENS_FACING_FRONT : LENS_FACING_BACK;
        this.mCameraId = LENS_FACING_BACK;
        this.mCameraManager = (CameraManager) context.getSystemService(Context.CAMERA_SERVICE);
        this.mCameraOrientationEventListener = new OrientationEventListener(context) {
            @Override
    ...
    public void openCamera(int width, int height) {
        ...
        try {
            Log.d(TAG, "screen size:" + context.getResources().getDisplayMetrics().widthPixels + "*" + context.getResources().getDisplayMetrics().heightPixels);
            mCameraCharacteristics = mCameraManager.getCameraCharacteristics(Integer.toString(mCameraId));

            Integer faceDetectCount = mCameraCharacteristics.get(STATISTICS_INFO_MAX_FACE_COUNT);             //同时检测到人脸的数量
            int[] faceDetectModes = mCameraCharacteristics.get(STATISTICS_INFO_AVAILABLE_FACE_DETECT_MODES);  //人脸检测的模式
            // 检查是不是支持人脸识别
            mFaceDetectMode = CaptureRequest.STATISTICS_FACE_DETECT_MODE_OFF;   //不支持人脸检测
            if (faceDetectModes != null) {
                for (int mode : faceDetectModes) {
                    ...
                    if (mode == CaptureRequest.STATISTICS_FACE_DETECT_MODE_SIMPLE && mFaceDetectMode != CaptureRequest.STATISTICS_FACE_DETECT_MODE_FULL) {
                        mFaceDetectMode = CaptureRequest.STATISTICS_FACE_DETECT_MODE_SIMPLE;
                    }
                }
            }
            if (mFaceDetectMode == CaptureRequest.STATISTICS_FACE_DETECT_MODE_OFF || faceDetectCount == null || faceDetectCount.equals(0)) {
                //todo
                Log.e(TAG, "该设备不支持人脸识别");
                return;
            }

            StreamConfigurationMap configurationMap = mCameraCharacteristics.get(SCALER_STREAM_CONFIGURATION_MAP);
            ...
            if (configurationMap != null) {
                ...
                // 设置人脸识别数据转换矩阵
                Rect activeArraySizeRect = mCameraCharacteristics.get(SENSOR_INFO_ACTIVE_ARRAY_SIZE); //获取成像区域

                if (activeArraySizeRect != null) {
                    Log.d(TAG, "activeArraySizeRect size: left " + activeArraySizeRect.left + " ,top " + activeArraySizeRect.top + " ,right " + activeArraySizeRect.right + " ,bottom " + activeArraySizeRect.bottom);
                    float scaledWidth = 1.0f * width / activeArraySizeRect.width();
                    float scaledHeight = 1.0f * height / activeArraySizeRect.height();
                    mFaceDetectMatrix.setRotate(getRotation());
                    mFaceDetectMatrix.postScale(isFrontCamera ? -scaledWidth : scaledWidth, scaledHeight);
    ...
    public void bindPreviewOnSurfaceView(SurfaceHolder holder) {
        mPreviewSurface = holder.getSurface();
    }

    public void bindPreviewOnTextureView(SurfaceTexture surfaceTexture) {
    ...
    private void initPreviewRequest() {
        try {
            ...
            Log.i(TAG, "人脸支持模式:" + mFaceDetectMode);
//            mPreviewRequestBuilder.set(CaptureRequest.STATISTICS_FACE_DETECT_MODE, mFaceDetectMode); // 设置人脸检测模式
            mPreviewRequestBuilder.set(CaptureRequest.STATISTICS_FACE_DETECT_MODE, CaptureRequest.STATISTICS_FACE_DETECT_MODE_FULL); // 设置人脸检测模式
            ...
            mCaptureSession.setRepeatingRequest(mPreviewRequest, new CameraCaptureSession.CaptureCallback() {
                ...
                public void onCaptureCompleted(@NonNull CameraCaptureSession session, @NonNull CaptureRequest request, @NonNull TotalCaptureResult result) {
                    ...
                    for (Face face : faces) {
                        ...
                        RectF rawFaceRect = new RectF(1.0f * left, 1.0f * top, 1.0f * right, 1.0f * bottom);
                        mFaceDetectMatrix.mapRect(rawFaceRect);
                        rawFaceRect = isFrontCamera ? rawFaceRect : new RectF(rawFaceRect.left, rawFaceRect.top - mPreviewSize.getWidth(), rawFaceRect.right, rawFaceRect.bottom - mPreviewSize.getWidth());
                        mFacesRect.add(rawFaceRect);

                        Log.i(TAG, String.format(Locale.getDefault(), "原始人脸位置:[%d * %d]==> 上[%d]   下[%d]   左[%d]   右[%d],  得分[%d]",
                                bounds.width(), bounds.height(), bounds.top, bounds.bottom, bounds.left, bounds.right, face.getScore()));
                        Log.i(TAG, String.format(Locale.getDefault(), "转换后人脸位置:[%.1f * %.1f]==> 上[%.1f]   下[%.1f]   左[%.1f]   右[%.1f],  得分[%d]",
                                rawFaceRect.width(), rawFaceRect.height(), rawFaceRect.top, rawFaceRect.bottom, rawFaceRect.left, rawFaceRect.right, face.getScore()));

                        Log.i(TAG, "左眼数据：" + (face.getLeftEyePosition() == null ? "无" : face.getLeftEyePosition().toString()));
                        Log.i(TAG, "右眼数据：" + (face.getRightEyePosition() == null ? "无" : face.getRightEyePosition().toString()));
                        Log.i(TAG, "嘴巴数据：" + (face.getMouthPosition() == null ? "无" : face.getMouthPosition().toString()));
                    }
                    // todo 回调出去结果
                }
            }, mBackgroundHandler);
    ...
    private int getJpegOrientation(int deviceOrientation) {
        if (deviceOrientation == android.view.OrientationEventListener.ORIENTATION_UNKNOWN)
            return 0;
        int sensorOrientation = mCameraCharacteristics.get(SENSOR_ORIENTATION);
        // Round device orientation to a multiple of 90
        deviceOrientation = (deviceOrientation + 45) / 90 * 90;
        // Reverse device orientation for front-facing cameras
        boolean facingFront = mCameraCharacteristics.get(LENS_FACING) == LENS_FACING_FRONT;
    ...
    public void handleZoom(boolean isZoomIn) {
        ...
        int maxZoom = mCameraCharacteristics.get(SCALER_AVAILABLE_MAX_DIGITAL_ZOOM).intValue()
                * 10;
        Log.d(TAG, "handleZoom: maxZoom: " + maxZoom);
        Rect rect = mCameraCharacteristics.get(SENSOR_INFO_ACTIVE_ARRAY_SIZE);
    ...
    public void focusOnPoint(double x, double y, int width, int height) {
        ...
        if (cropRegion == null) {
            ...
            cropRegion = mCameraCharacteristics.get(SENSOR_INFO_ACTIVE_ARRAY_SIZE);
        }
    ...
    private void startBackgroundThread() {
        if (mBackgroundThread == null || mBackgroundHandler == null) {
            ...
            mBackgroundThread = new HandlerThread(TAG_CAMERA);
